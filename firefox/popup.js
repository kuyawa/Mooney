...
function showBalances(assets) {
    ...
    //table.tBodies[0].children.remove(); // Clear table
    ...
    for (var i = 0; i < assets.length; i++) {
        ...
        row.setAttribute('id',     item.asset);
        row.setAttribute('issuer', item.issuer);
        cell0.textContent = item.asset;
        cell1.textContent = item.name;
        cell2.textContent = money(item.balance, 2, true);
    }
}
...
function showHistory(list) {
    ...
    for (var i = 0; i < list.length; i++) {
        ...
        cell2.setAttribute('color', color);
        cell3.textContent = asset;
    }
