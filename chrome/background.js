...
chrome.runtime.onInstalled.addListener(function(details) {
	...
	chrome.storage.sync.get('appkey', function(data) {
		var appkey = data.appkey;

		if(!appkey){  /* FIRST TIME */
			var appkey = generateKey();

		    chrome.storage.sync.set({appkey: appkey}, function() {
    ...
    chrome.declarativeContent.onPageChanged.removeRules(undefined, function() {
        chrome.declarativeContent.onPageChanged.addRules([{
            ...
            //conditions: [new chrome.declarativeContent.PageStateMatcher({pageUrl: {schemes: ['web+stellar']}})],
            //conditions: [new chrome.declarativeContent.PageStateMatcher({pageUrl: {hostEquals: '<all_urls>'}})],
...
function saveAccount(secret) {
    ...
    encryptKey(secret, encrypted=>{
        ...
        chrome.storage.sync.set({token: encrypted}, function() {
